...
- date: January 2022
- description: Creating from a CSV a series of statements for places for a Python function
- input: CSV file
- output: Python file
- usage :
    ======
...
with open(sys.argv[1], 'r', encoding='UTF-8') as csvfile:
    ...
    for row in index:
        ...
        encoding = f"""
        text = re.sub({identifier}, r'<placeName ref="#{identifier}" type="pec">\g<0></placeName>', text)
        """
